...
def _taz(data_paths):
    taz = pd.read_csv(config.data_path+ data_paths['taz_interstate_file']).sort_values(['SPRT_ID']).reset_index(drop = True)[0:max_zone]
    
    taz = taz.merge(pd.read_csv(config.data_path + data_paths['taz_file']).sort_values(['SPRT_ID'])[0:max_zone][['SPRT_ID','BOSTON_NB','TOWN']], how = 'left') 
    ...
    taz_parking = pd.read_csv(config.data_path + data_paths['taz_parking_file']).fillna(0)
    taz_zonal = pd.read_csv(config.data_path + data_paths['taz_zonal_file']).sort_values('SPRT_ID')
...
def _study_area(taz):
    seaport = ['South Station', 'Seaport Blvd', 'Design Center',
